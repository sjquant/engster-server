...
def _get_smi_line_info(
    time: str, line: str, lang: str
...
def get_srt_lines(text: str) -> list:
    ...
    re_sync = re.compile(r"(\d\d:\d\d:\d\d),\d+\s*-->\s*\d\d:\d\d:\d\d,\d+\s*")
...
def combine_eng_kor(df_a: pd.DataFrame, df_b: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.rename(columns={"line_eng": "line", "line_kr": "translation"})[
        ["line", "translation"]
    ]
    return df
